...
CAMERA_BROWSER_PERMISSION = "camera_browser_permission"
TIMER_LOGOUT_RESET = "timer_logout_reset"
...
_PATTERNS: tuple[tuple[str, tuple[str, ...]], ...] = (
    ...
    (
        ...
        (
            ...
            "camera blocked",
            "browser is blocking the camera",
...
def focused_guidance(subissue: str, lang: str, fix_failed: bool = False) -> Optional[Dict[str, Any]]:
    ...
    if zh:
        data = {
            ...
            BLACK_SCREEN_APP_RESTART: {
                "lead": "黑屏/app 自重启：只刷新/重新加载一次并等待；若无效，源文件未记录修复方法，保存证据并升级。",
                "steps": [
                    "确认是黑屏、app 自重启或进度丢失。",
                    "刷新/重新加载一次并等待；如果恢复，继续并记录。",
                    "如果无效，源文件记录该问题但没有记录修复方法。",
                    "保存照片/视频、发生时间、课程步骤，以及供电/显示/Bluetooth/tablet-PAD 状态。",
                    "升级工程师/厂商；如影响课程，通知主管。",
                ],
                ...
                "do_not_decide_without_approval": ["不要尝试未记录的重置、恢复或校准步骤。"],
            },
            CAMERA_BROWSER_PERMISSION: {
                "lead": "摄像头/浏览器权限问题：来源没有记录设备会话权限修复；只检查来源支持的基本 app 内权限提示，无法解决就截图并升级。",
                "steps": [
                    "确认是否是 camera permission denied / 浏览器阻止摄像头。",
                    "只在 SOP 支持的情况下检查基本 app 内权限提示。",
                    "如仍无效，截图/记录准确报错文字。",
                    "升级工程师/厂商或技术支持；如影响课程，通知主管。",
                    "不要重装、刷机、更换设备，或编造浏览器/Wi-Fi/校准修复。",
                ],
                "information_to_collect": ["准确报错文字。", "发生时间与课程步骤。", "设备/浏览器状态。"],
                "evidence_requested": ["权限报错截图。", "浏览器/app 状态截图。", safe_photo],
                "contacts": ["工程师 / 厂商或技术支持。", "主管（影响课程时）。"],
                "next_actions": ["截图记录准确报错。", "升级工程师/厂商或技术支持。"],
                "do_not_decide_without_approval": ["不要执行未支持的技术修复。", "不要重装、刷机或更换设备。"],
            },
            TIMER_LOGOUT_RESET: {
                "lead": "Timer / logout / reset 行为需要 ALLCPR 确认。来源未说明这是正常行为；按黑屏/加载问题记录并升级。",
                "steps": [
                    "记录 timer、logout 或 reset 的准确表现和时间。",
                    "保存照片/视频或截图，以及课程步骤。",
                    "按 Section 8 处理：不要编造修复，升级确认。",
                    "联系 ALLCPR support 确认是否为预期行为。",
                    "未确认前，不要告诉学员这是正常现象。",
                ],
                "information_to_collect": ["准确表现。", "发生时间。", "课程步骤。"],
                "evidence_requested": ["timer/logout/reset 截图或视频。", safe_photo],
                "contacts": ["ALLCPR support。", "主管（影响课程时）。"],
                "next_actions": ["保存证据并升级 ALLCPR support。"],
                "do_not_decide_without_approval": ["不要编造修复。", "未经 ALLCPR 确认不要说这是正常行为。"],
            },
            COMPLETION_PHOTO: {
        ...
        data = {
            ...
            BLACK_SCREEN_APP_RESTART: {
                "lead": "Black screen/app restart: reload or refresh once and wait. If it does not recover, the source records no documented fix; save evidence and escalate.",
                "steps": [
                    "Confirm whether this is black screen, app self-restart, or lost progress.",
                    "Reload/refresh once and wait; if it recovers, continue and note it.",
                    "If it does not recover, the source records this issue but no documented fix.",
                    "Save photo/video, exact time, course step, and power/display/Bluetooth/tablet-PAD status.",
                    "Escalate to engineer/vendor; notify supervisor if class is affected.",
                ],
                ...
                "do_not_decide_without_approval": ["Do not try undocumented reset, recovery, or calibration steps."],
            },
            CAMERA_BROWSER_PERMISSION: {
                "lead": "Camera/browser permission issue: the source records no device-session permission fix. Check only a supported basic in-app permission prompt, then document and escalate if unresolved.",
                "steps": [
                    "Confirm whether this is camera permission denied or the browser blocking the camera.",
                    "Check a basic in-app permission prompt only when the SOP supports it for the situation.",
                    "If unresolved, capture the exact error text or screenshot.",
                    "Escalate to engineer/vendor or tech support; notify supervisor if class is affected.",
                    "Do not reinstall, re-flash, change devices, or invent browser/Wi-Fi/calibration fixes.",
                ],
                "information_to_collect": ["Exact error text.", "Time and course step.", "Device/browser state."],
                "evidence_requested": ["Permission error screenshot.", "Browser/app state screenshot.", safe_photo],
                "contacts": ["Engineer/vendor or tech support.", "Supervisor (when class is affected)."],
                "next_actions": ["Capture the exact error.", "Escalate to engineer/vendor or tech support."],
                "do_not_decide_without_approval": ["Do not perform unsupported technical fixes.", "Do not reinstall, re-flash, or change devices."],
            },
            TIMER_LOGOUT_RESET: {
                "lead": "Timer / logout / reset behavior needs ALLCPR confirmation. The source does not document it as intended behavior; document and escalate before calling it normal.",
                "steps": [
                    "Record the exact timer, logout, or reset behavior and time.",
                    "Save photo/video or screenshot evidence and the course step.",
                    "Treat it like Section 8: do not invent a fix; document and escalate.",
                    "Escalate to ALLCPR support to confirm whether it is expected behavior.",
                    "Do not tell students it is normal unless ALLCPR confirms it.",
                ],
                "information_to_collect": ["Exact behavior.", "Time.", "Course step."],
                "evidence_requested": ["Timer/logout/reset screenshot or video.", safe_photo],
                "contacts": ["ALLCPR support.", "Supervisor (when class is affected)."],
                "next_actions": ["Save evidence and escalate to ALLCPR support."],
                "do_not_decide_without_approval": ["Do not invent a fix.", "Do not call it normal without ALLCPR confirmation."],
            },
            COMPLETION_PHOTO: {
