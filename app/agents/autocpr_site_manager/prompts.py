...
GUIDANCE_EN: Dict[str, Dict[str, object]] = {
    ...
    "smart_manikin_troubleshooting": {
        ...
        "steps": [
            "Bluetooth won't connect when unplugged (source freq 4) → use the power strip to keep the manikin + PAD powered; if it still won't connect while plugged in → restart the manikin (the source-recorded fix). Note: while powered-but-unplugged the PAD may show 'connected' but TRAINING receives no data.",
            "Can't find the room / wrong floor (freq 2-3) → classroom-finding video + room/gate passcode + add signage (recorded fix: add 2F signage).",
            "Forgot the completion photo (freq 2) → photograph the 'All Session Done/Pass' screen and email it to support@allcpr.org.",
            "Black screen / app self-restart loses progress (freq 3) → the source records the issue, but no documented fix is recorded; do not improvise an undocumented recovery step → needs engineer/vendor confirmation.",
        ],
        "information_to_collect": [
...
GUIDANCE_ZH: Dict[str, Dict[str, object]] = {
    ...
    "smart_manikin_troubleshooting": {
        ...
        "steps": [
            "Bluetooth 连不上（不插电时，源文件 freq 4）→ 用插线板让假人+PAD 保持供电；若插电仍连不上 → 重启假人（源文件记录的方法）。注意：不插电但开机时 PAD 可能显示 connected，但 TRAINING 收不到数据。",
            "找不到房间/走错楼层（freq 2–3）→ 课堂查找视频 + 房间/大门密码 + 增加指示牌（源文件记录：“二楼增添指示牌”）。",
            "忘记完成截图（freq 2）→ 提醒拍“All Session Done/Pass”界面并邮件到 support@allcpr.org。",
            "黑屏 / app 自重启导致进度丢失（freq 3）→ 源文件记录了该问题但未记录修复方法，不要自行编造重启/恢复步骤 → needs engineer/vendor confirmation。",
        ],
